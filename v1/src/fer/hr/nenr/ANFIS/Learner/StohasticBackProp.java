 ...
 */
public class StohasticBackProp extends Learner{
    public StohasticBackProp(Double faktor,double eps, int maxIter){
        ...
        for(int iter = 0; iter<this.maxIter;){
            ...
            for(List<Double> vektor:data.keySet()) {
                ...
                for (int j = 0; j < izlJ; j++) {
                    double localDer = 0;
                    for (int o = 0; o < derivationsForBefore.size(); o++) {
                        //System.out.println("tezina "+o+" vrijednost = "+network.getWeight(1, o, j));
            ...
            if (trainError < this.eps) {
                System.out.println("prije izasao "+ iter);
                return;
            }
        }
    }
